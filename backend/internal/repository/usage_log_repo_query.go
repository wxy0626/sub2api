...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		// 请求大小列允许 NULL，以兼容迁移前的历史 usage_logs。
	...
	log := &service.UsageLog{
		...
		RequestBodyBytes:          nullableInt64Ptr(requestBodyBytes),
		MaxRequestBodyBytes:       nullableInt64Ptr(maxRequestBodyBytes),
		CreatedAt:                 createdAt,
	}
...
// nullableInt64Ptr 将数据库中的可空 BIGINT 转换为 service 层使用的可空指针。
