...
// 验证原生 WS 在发现部分 replay 前缀时会在上游写入前拒绝本轮请求。
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_RejectsPartialReplayBeforeUpstreamWrite(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// 上游准备两个终态事件；第二个事件若被消费，说明歧义请求已经被发送。
	upstreamConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_partial_prefix_first","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
			[]byte(`{"type":"response.completed","response":{"id":"resp_partial_prefix_should_not_send","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	...
	account := &Account{
		ID:          131,
		Name:        "openai-ingress-partial-replay-reject",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"input":[{"type":"input_text","text":"system"},{"type":"input_text","text":"user"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_partial_prefix_first", gjson.GetBytes(firstTurn, "response.id").String())

	// 本轮只复用上一轮输入的前缀，不能静默把旧 replay 与当前输入继续拼接。
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"previous_response_id":"resp_partial_prefix_first","input":[{"type":"input_text","text":"system"},{"type":"input_text","text":"new"}]}`)

	select {
	case serverErr := <-serverErrCh:
		require.Error(t, serverErr)
		require.ErrorContains(t, serverErr, "partially overlaps previous replay input")
	case <-time.After(5 * time.Second):
		t.Fatal("等待 partial replay 被拒绝超时")
	}

	upstreamConn.mu.Lock()
	writeCount := len(upstreamConn.writes) // 记录实际写入上游的 response.create 数量。
	upstreamConn.mu.Unlock()
	require.Equal(t, 1, writeCount, "partial replay 被拒绝后不应向上游发送第二轮")
}
