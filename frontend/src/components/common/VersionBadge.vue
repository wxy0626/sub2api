<template>
  <div class="relative">
    ...
    <template v-if="isAdmin">
      ...
      >
        <span v-if="currentVersion" class="font-medium">{{ currentVersion }}</span>
        <span
          v-else
      ...
      <transition name="dropdown">
        <div
          ...
          :class="rollbackPanelOpen ? 'w-80' : 'w-64'"
        >
          ...
          <div class="p-4">
            ...
            <template v-else>
              ...
              <!-- Priority 2: Update success - manual restart or scheduled local image replacement -->
              <div v-else-if="updateSuccess && (needRestart || rollbackRestartScheduled)" class="space-y-2">
                <div
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-green-600/70 dark:text-green-400/70">
                      {{ rollbackRestartScheduled ? t('version.rollbackReconnecting') : t('version.restartRequired') }}
                    </p>
                  </div>
                </div>
                ...
                <p v-else class="text-center text-xs text-gray-500 dark:text-dark-400">
                  {{ t('version.rollbackAutoRestarting', { seconds: restartCountdown }) }}
                </p>
              </div>

              <!-- 上游版本只用于状态提示；安装和回退只使用个人 Git/镜像仓库版本。 -->
              <div v-else class="space-y-2">
                ...
                >
                  ...
                  {{ hasUpdate ? t('version.upstreamUpdateAvailable', { version: latestVersion }) : t('version.viewRelease') }}
                </a>

                <!-- Personal Git image deployment entry -->
                <div class="border-t border-gray-100 pt-2 dark:border-dark-700">
                  ...
                  <transition name="rollback">
                    <div v-if="rollbackPanelOpen" class="mt-2 space-y-2">
                      <!-- Loading verified personal versions -->
                      <div
                        v-if="rollbackImagesLoading"
                      ...
                      <!-- Load error + retry -->
                      <div v-else-if="rollbackImagesError" class="space-y-2">
                        <p
                      ...
                      <!-- No verified personal versions available -->
                      <p
                        v-else-if="rollbackImages.length === 0"
                        class="py-3 text-center text-xs text-gray-400 dark:text-dark-500"
                      >
                        {{ t('version.noRollbackVersions') }}
                      </p>

                      <!-- Personal latest deployment and historical rollback list -->
                      <template v-else>
                        <p class="px-0.5 text-[11px] text-gray-400 dark:text-dark-500">
                          {{ t('version.personalVersionSource') }}
                        </p>

                        <button
                          v-if="latestPersonalVersion"
                          @click="handleDeployLatestPersonalVersion"
                          :disabled="deployingLatest || rollingBack"
                          class="flex w-full items-center justify-center gap-2 rounded-lg bg-primary-500 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          <svg v-if="deployingLatest" class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                          </svg>
                          <Icon v-else name="download" size="sm" :stroke-width="2" />
                          {{ deployingLatest ? t('version.deployingPersonalLatest') : t('version.deployPersonalLatest', { version: latestPersonalVersion.tag }) }}
                        </button>

                        <p v-if="latestPersonalVersion" class="px-0.5 text-[11px] text-gray-400 dark:text-dark-500">
                          {{ latestPersonalVersion.reference }} · {{ latestPersonalVersion.commit.slice(0, 12) }}
                        </p>

                        <template v-if="rollbackVersions.length > 0">
                          <p class="border-t border-gray-100 pt-2 text-[11px] text-gray-400 dark:border-dark-700 dark:text-dark-500">
                            {{ t('version.personalRollbackVersions') }}
                          </p>
                          <button
                            v-for="item in rollbackVersions"
                            :key="item.digest"
                            @click="selectRollbackImage(item.tag)"
                            :disabled="rollingBack || deployingLatest"
                            ...
                            :class="selectedRollbackImageID === item.tag ? 'border-amber-300 bg-amber-50 shadow-sm dark:border-amber-700 dark:bg-amber-900/20' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50 dark:border-dark-700 dark:hover:border-dark-600 dark:hover:bg-dark-700/40'"
                          >
                            <span class="text-sm font-semibold" :class="selectedRollbackImageID === item.tag ? 'text-amber-700 dark:text-amber-300' : 'text-gray-700 dark:text-dark-200'">{{ item.tag }}</span>
                            <span class="text-[11px] tabular-nums text-gray-400 dark:text-dark-500">{{ item.commit.slice(0, 12) }}</span>
                          </button>
                        </template>

                        <transition name="rollback">
                          <div v-if="selectedRollbackImage" class="space-y-2">
                            <p class="px-0.5 text-[11px] text-gray-400 dark:text-dark-500">
                              {{ selectedRollbackImage.reference }}
                            </p>
                            ...
                            >
                              ...
                              />
                              {{ t('version.rollbackPersonalImageWarning') }}
                            </p>
                              ...
                              <span>{{
                                rollingBack
                                  ...
                                  : t('version.rollbackConfirm', {
                                      image: selectedRollbackImage.tag
                                    })
                              }}</span>
                            </button>
                          </div>
...
// 用户 Git tag 与 OCI digest 验证后的部署/回退状态
const rollbackPanelOpen = ref(false)
const rollbackImages = ref<PersonalDeploymentVersion[]>([])
const rollbackImagesLoading = ref(false)
const rollbackImagesError = ref('')
const selectedRollbackImageID = ref('')
const rollingBack = ref(false)
const deployingLatest = ref(false)
const rollbackError = ref('')
const rollbackRestartScheduled = ref(false)

const selectedRollbackImage = computed(() =>
  rollbackImages.value.find((item) => item.tag === selectedRollbackImageID.value)
)
const latestPersonalVersion = computed(() => rollbackImages.value[0])
const rollbackVersions = computed(() => rollbackImages.value.slice(1))
...
async function refreshVersion(force = true) {
  ...
  needRestart.value = false
	rollbackRestartScheduled.value = false
  resetRollbackState()
...
function resetRollbackState() {
  ...
  rollbackImages.value = []
  rollbackImagesError.value = ''
  selectedRollbackImageID.value = ''
  rollbackError.value = ''
	rollbackRestartScheduled.value = false
}
...
async function toggleRollbackPanel() {
  ...
  // 自有镜像版本独立于上游 Release 字符串；自定义 -dev 构建同样可部署已发布 Git tag。
  if (
    rollbackPanelOpen.value &&
    rollbackImages.value.length === 0 &&
    !rollbackImagesLoading.value
  ) {
    await loadRollbackImages()
  }
}

async function loadRollbackImages() {
  if (!isAdmin.value) return
  rollbackImagesLoading.value = true
  rollbackImagesError.value = ''
  try {
    const data = await getPersonalDeploymentVersions()
    rollbackImages.value = data.versions || []
  } catch (error: unknown) {
    const err = error as { response?: { data?: { message?: string } }; message?: string }
    rollbackImagesError.value =
      err.response?.data?.message || err.message || t('version.loadVersionsFailed')
  } finally {
    rollbackImagesLoading.value = false
  }
}

function selectRollbackImage(tag: string) {
  if (rollingBack.value || deployingLatest.value) return
  rollbackError.value = ''
  selectedRollbackImageID.value = selectedRollbackImageID.value === tag ? '' : tag
}

async function handleRollback() {
  if (!isAdmin.value) return
  if (rollingBack.value || !selectedRollbackImage.value) return

  rollingBack.value = true
  rollbackError.value = ''

  try {
    const result = await rollbackAPI(selectedRollbackImage.value.tag, selectedRollbackImage.value.digest)
    successKind.value = 'rollback'
    updateSuccess.value = true
    needRestart.value = result.need_restart
    rollbackRestartScheduled.value = Boolean(result.restart_scheduled)
    rollbackPanelOpen.value = false
    // Clear version cache so the next check reflects the rolled-back version
    appStore.clearVersionCache()
		if (rollbackRestartScheduled.value) {
			startRollbackReconnectCountdown()
		}
  } catch (error: unknown) {
    const err = error as { response?: { data?: { message?: string } }; message?: string }
...
async function handleDeployLatestPersonalVersion() {
  if (!isAdmin.value || deployingLatest.value || rollingBack.value) return

  deployingLatest.value = true
  rollbackError.value = ''
  updateSuccess.value = false

  try {
    const result = await deployLatestPersonalVersion()
    successKind.value = 'update'
    updateSuccess.value = true
    needRestart.value = result.need_restart
    rollbackRestartScheduled.value = Boolean(result.restart_scheduled)
    ...
    const err = error as { response?: { data?: { message?: string } }; message?: string }
    rollbackError.value = err.response?.data?.message || err.message || t('version.deployPersonalLatestFailed')
  } finally {
    deployingLatest.value = false
  }
}

function startRollbackReconnectCountdown() {
