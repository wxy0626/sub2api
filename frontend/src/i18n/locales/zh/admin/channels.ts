export default {
    ...
    channelMonitor: {
      ...
      form: {
        ...
        provider: '平台',
        source: '监控来源',
        sourceExternal: '第三方 API',
        sourceAccount: '已有账号',
        account: '账号',
        accountPlaceholder: '选择要监控的账号',
        accountHint: '使用账号已保存的凭据进行检测，不会把本地 API Key 发送到第三方。',
        accountRequired: '请选择要监控的账号',
        linkedAccount: '关联账号',
        linkedAccountPlaceholder: '选择关联账号',
        linkedAccountHint: '使用已保存凭据执行监控。',
        linkedAccountMissing: '关联账号不存在或已被删除。',
        linkedAccountEmpty: '当前没有可关联的账号。',
        apiMode: 'OpenAI 协议',
        ...
        checkModeProbe: '主动探测',
        checkModeProbeHint: '定时向上游发送请求，检查渠道是否可用。',
        checkModeQuota: '配额模式',
        checkModeQuotaHint: '仅查询关联账号的余额或配额，不发送完整模型请求。',
        checkModeQuotaProbe: '配额主动探测',
        checkModeQuotaProbeHint: '通过配额接口主动检查账号剩余额度。',
        endpoint: '上游地址',
        endpointPlaceholder: 'https://api.example.com',
        useCurrentDomain: '使用当前服务',
        endpointHTTPSHint: '仅支持 HTTPS 根地址，例如 https://api.example.com。',
        endpointHTTPSRequired: '上游地址必须是 HTTPS 根地址，例如 https://api.example.com',
        apiKey: 'API Key',
        apiKeyPlaceholder: '请输入 API Key',
