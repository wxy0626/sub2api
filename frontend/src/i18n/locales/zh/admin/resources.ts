export default {
    scheduledTests: {
      ...
      running: '运行中',
      schedule: '定时测试',
      cronHelp: '标准 5 字段 cron 表达式（默认 0 * * * *，每 60 分钟运行一次）',
      cronTooltipTitle: 'Cron 表达式示例：',
      cronTooltipMeaning: '用于定义自动执行测试的时间规则，格式依次为：分钟 小时 日 月 星期。',
      cronTooltipExampleEvery60Min: '0 * * * *：每 60 分钟运行一次',
      ...
      cronTooltipRange: '推荐填写范围：使用标准 5 字段 cron；如果只是健康检查，建议从每 60 分钟或每天固定时间开始，不建议一开始就设置过高频率。',
